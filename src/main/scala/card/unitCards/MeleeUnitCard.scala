 ...
 * @version 1.0
 */
class MeleeUnitCard(name: String, strength: Int) extends AbstractUnitCard(name, strength) with Equals{
   ...
   * }}}
   */
  override def canEqual(that: Any): Boolean = that.isInstanceOf[MeleeUnitCard]
  ...
  override def hashCode: Int = Objects.hash(classOf[MeleeUnitCard], name, strength)
}